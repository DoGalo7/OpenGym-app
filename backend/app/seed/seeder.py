...
from app.models import Exercise, FixedWod, FixedWodMovement, PredefinedWod, PredefinedWodMovement
...
def run_seed(db: Session) -> None:
    ...
    for data in FIXED_WODS:
        entry = {k: v for k, v in data.items() if k != "movements"}
        wod = existing_fixed_by_name.get(data["name"])
        if wod:
            for field, value in entry.items():
                setattr(wod, field, value)
            db.query(FixedWodMovement).filter_by(fixed_wod_id=wod.id).delete()
        else:
            wod = FixedWod(**entry)
            db.add(wod)
        db.flush()  # assign/confirm wod.id for the movements below
        for position, movement in enumerate(data.get("movements", [])):
    ...
    for entry in PREDEFINED_WODS:
        ...
        if wod:
            ...
            wod.is_power = entry.get("is_power", False)
